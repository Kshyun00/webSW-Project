...
app.post("/boards", upload.single("image"), (req, res) => {
  ...
  if (!email) {
    return res
      .status(401)
      .json({ message: "쿠키가 없습니다. 로그인이 필요합니다." });
  }
...
app.delete("/boards/:postId", (req, res) => {
  const { postId } = req.params; // URL에서 postId 추출
  const userEmail = req.cookies.email; // 쿠키에서 사용자 이메일 추출
  const isMaster = req.cookies.master; // 쿠키에서 마스터 여부 추출
...
app.post("/boards/:id/comments", (req, res) => {
  ...
  const email = req.cookies.email; // 쿠키에서 사용자 이메일 추출
...
app.delete("/comments/:id", (req, res) => {
  ...
  const userEmail = req.cookies.email; // 쿠키에서 사용자 이메일 추출
  const isMaster = req.cookies.master; // 마스터 여부 확인
